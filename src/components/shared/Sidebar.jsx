...
const SidebarItem = ({ icon: Icon, label, href, active, isCollapsed }) => (
    <Link
        to={href}
    ...
    >
        ...
        {!isCollapsed ? (
            <span className={`text-[14px] uppercase tracking-wider whitespace-nowrap transition-all duration-300 ${active ? 'font-black' : 'font-bold'}`}>
                {label}
            </span>
        ) : (
            /* Custom Tooltip for Collapsed State */
            <div className="absolute left-full ml-4 px-3 py-2 bg-charcoal dark:bg-primary text-white text-[11px] font-black uppercase tracking-widest rounded-lg opacity-0 pointer-events-none group-hover:opacity-100 group-hover:translate-x-1 transition-all z-50 shadow-xl whitespace-nowrap border border-white/10">
                {label}
                {/* Tooltip Arrow */}
...
const Sidebar = ({ isCollapsed, onToggle }) => {
    ...
    return (
        <aside className={`${isCollapsed ? 'w-20' : 'w-64'} h-screen border-r border-gray-100 shadow-[3px_0px_2px_rgba(0,0,0,0.1)] dark:border-gray-800 bg-[#F9F9FB] dark:bg-[#0D0B14] flex flex-col fixed left-0 top-0 z-50 transition-all duration-300 ease-in-out`}>
            {/* Header & Toggle */}
            ...
            {/* Navigation */}
            <nav className={`flex-1 ${isCollapsed ? 'mt-4' : 'mt-8'}`}>
                {navItems.map((item) => (
                    <SidebarItem
                        key={item.label}
                        {...item}
            ...
            <div className="relative mt-auto p-4" ref={menuRef}>
                ...
                {isMenuOpen && (
                    <div className={`absolute left-full ml-3 bottom-4 bg-white dark:bg-gray-900 rounded-3xl shadow-[3px_3px_0px_rgba(0,0,0,0.15)] border border-gray-100 dark:border-gray-800 p-2 z-[60] transition-all animate-in fade-in slide-in-from-left-4 duration-300 w-56`}>
                        {/* Header Section */}
                        ...
                        <div className="flex flex-col gap-0.5">
                            <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-primary/5 text-gray-600 dark:text-gray-400 hover:text-primary transition-all text-xs font-black uppercase tracking-wider group">
                                <Sparkles size={16} className="group-hover:scale-110 transition-transform" />
                                Upgrade (Pro)
                            </button>
                            <div className="h-[1px] bg-gray-50 dark:bg-gray-800/50 my-1 mx-2" />
                            <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 text-gray-600 dark:text-gray-400 transition-all text-xs font-black uppercase tracking-wider">
                                <BadgeCheck size={16} />
                                Account
                            </button>
                            <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 text-gray-600 dark:text-gray-400 transition-all text-xs font-black uppercase tracking-wider">
